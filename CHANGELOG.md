...
- Added direct local OTA mode in firmware update API: `mode=url` with `url` and `filesystemUrl`.
- Added OTA UI controls to update from custom firmware/filesystem URLs without publishing a GitHub release.
- Added local helper script `scripts/ota/local_ota_update.sh` for build + local HTTP + OTA trigger flow.
- Extended integration tests for new OTA mode and UTF-8-safe growth schedule name truncation.
